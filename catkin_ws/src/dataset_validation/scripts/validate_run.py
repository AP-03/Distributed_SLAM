...
def _required_topics(robot_id, profile):
    profiles = {
        "one_robot_rigid": [
            "/tf",
            "/tf_static",
    ...
    if profile not in profiles:
        raise ValueError("unknown validation profile: %s" % profile)
    return profiles[profile]


def _default_bag_name(robot_id, profile):
    if profile.startswith("mocap_only"):
        return "mocap.bag"
    return "%s.bag" % robot_id
...
def main():
    ...
    parser.add_argument(
        "--profile",
        default="one_robot_marker_bootstrap",
        choices=["one_robot_rigid", "one_robot_marker_bootstrap", "mocap_only_marker_bootstrap", "mocap_only_rigid"],
    )
    parser.add_argument("--required_topic", action="append", default=None)
    args = parser.parse_args()

    run_dir = os.path.expanduser(args.run_dir)
    bag_path = os.path.expanduser(args.bag) if args.bag else os.path.join(run_dir, "raw", _default_bag_name(args.robot_id, args.profile))
    ...
    required_topics = args.required_topic or _required_topics(args.robot_id, args.profile)
