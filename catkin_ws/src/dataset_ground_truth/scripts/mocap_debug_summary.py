...
def _format_table(title, entries):
    ...
    lines.append("| ID | Count | Duration s | Path m | Mean Cond | Min Cond | X Range | Y Range | Z Range |")
    lines.append("| ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |")
...
def summarize_bag(bag_path, markers_topic, rigids_topic):
        ...
        for topic, msg, t in bag.read_messages(topics=[markers_topic, rigids_topic]):
            ...
            if topic == markers_topic:
                for marker in msg.markers:
                    _update(led_stats[int(marker.id)], timestamp, marker.x, marker.y, marker.z, marker.cond)
            elif topic == rigids_topic:
                for rigid in msg.rigids:
                    _update(rigid_stats[int(rigid.id)], timestamp, rigid.x, rigid.y, rigid.z, rigid.cond)
    finally:
        bag.close()
...
def main():
    ...
    topic_counts, led_stats, rigid_stats = summarize_bag(bag_path, args.markers_topic, args.rigids_topic)
